...
def check_seq_permutations(sequence):
	# Runs check_seq on all versions of a sequence without one item
	# Returns 1 if any of them pass
	# Returns 0 if all of them fail
	for i in range(len(sequence)):
		if check_seq(sequence[:i] + sequence[i+1:]):
			return 1
	return 0
...
if __name__ == "__main__":
	...
	for sequence in sequences:
		if check_seq(sequence):
			total += 1
		else:
			total += check_seq_permutations(sequence)
